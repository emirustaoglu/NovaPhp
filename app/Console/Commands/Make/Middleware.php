...
class Middleware extends Command
{
    ...
    public function handle(): void
    {
        ...
        $newMiddlewaresPath = config('app.paths.middlewares') . "/" . $middlewareName . '.php';
        ...
        $namespace = config('app.namespaces.middlewares');
...
namespace " . $namespace . ";", // Dinamik namespace ekle
        ...
        if (file_put_contents($newMiddlewaresPath, $templateContent) !== false) {
            $this->info("Yeni middleware dosyası oluşturuldu: $middlewareName");
        } else {
