...
class Controller extends Command
{
    ...
    public function handle(): void
    {
        ...
        $newControllerPath = config('app.paths.controllers') . "/" . $controllerName . '.php';
        ...
        $namespace = config('app.namespaces.controllers');
...
namespace " . $namespace . ";", // Dinamik namespace ekle
                str_replace(
                    '%UseArea%',
                    "use Core\Controller;",
                    str_replace('%ClassArea%', "class " . $className . " extends Controller { }", $templateContent)
                )
            );
        ...
        if (file_put_contents($newControllerPath, $templateContent) !== false) {
            $this->info("Yeni controller dosyası oluşturuldu: $controllerName");
        } else {
